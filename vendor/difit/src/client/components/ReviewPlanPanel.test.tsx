...
describe('ReviewPlanPanel', () => {
  it('does not imply a Copilot request until the reviewer explicitly generates', () => {
    ...
    render(<ReviewPlanPanel view="canonical" onViewChange={vi.fn()} models={[]} model="" onModelChange={vi.fn()} hunks={hunks} record={null} stale={false} onGenerate={generate} onOpenHunk={vi.fn()} onPreviousHunk={vi.fn()} onNextHunk={vi.fn()} />);
  ...
  it('never applies a stale plan to the current diff', () => {
    render(<ReviewPlanPanel view="plan" onViewChange={vi.fn()} models={[]} model="" onModelChange={vi.fn()} hunks={hunks} stale record={{ state: 'ready', result: { entries: [{ hunkId: 'two', rank: 1, rationale: 'Check this first.' }] } }} onGenerate={vi.fn()} onOpenHunk={vi.fn()} onPreviousHunk={vi.fn()} onNextHunk={vi.fn()} />);
  ...
  it('renders saved plan entries and sends navigation to the selected hunk', () => {
    ...
    render(<ReviewPlanPanel view="plan" onViewChange={vi.fn()} models={[{ id: 'gpt-5', name: 'GPT-5' }, { id: 'claude', name: 'Claude' }]} model="gpt-5" onModelChange={changeModel} hunks={hunks} stale={false} record={{ state: 'ready', model: 'gpt-5', result: { entries: [{ hunkId: 'two', rank: 1, rationale: 'Check this first.' }] } }} onGenerate={vi.fn()} onOpenHunk={open} onPreviousHunk={vi.fn()} onNextHunk={vi.fn()} />);
